...
import com.ymj.rabbit.config.SimpleConfig;
...
public class SimpleSendController {
    ...
    public String send(@PathVariable("msg") String msg) {
        for (int count = 1; count <= 10; count++) {
            ...
            infoMap.put("queueName", SimpleConfig.SIMPLE_QUEUE);
            infoMap.put("messageId", String.valueOf(UUID.randomUUID()));
            infoMap.put("messageData", msg + "--" + count);
            infoMap.put("createTime", createTime);

            rabbitTemplate.convertAndSend(SimpleConfig.SIMPLE_QUEUE, infoMap);
        }
        return SimpleConfig.SIMPLE_QUEUE + "_消息发送成功";
    }
