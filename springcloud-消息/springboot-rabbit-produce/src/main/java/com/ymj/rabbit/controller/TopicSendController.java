...
public class TopicSendController {
    ...
    public String send(@PathVariable("msg") String msg) {
        for (int count = 1; count <= 10; count++) {
            ...
            infoMap.put("queueName", TopicConfig.TOPIC_QUEUE);
            infoMap.put("messageId", String.valueOf(UUID.randomUUID()));
            infoMap.put("messageData", msg + "--" + count);
            infoMap.put("createTime", createTime);

            rabbitTemplate.convertAndSend(TopicConfig.TOPIC_QUEUE, infoMap);
        }
        return TopicConfig.TOPIC_EXCHANGE_NAME + "_消息发送成功";
    }
